...
import com.devbu03.base.common.enums.ProductStateEnum;
...
import com.devbu03.base.service.productTemp.ProductTempServiceImpl;
...
public class ProductServiceImpl implements ProductService {
  ...
  private final ProductTempServiceImpl productTempService;
  ...
  public Page<ProductDTO> findAllWithRole(Authentication authentication, int page, int size) {
    ...
    return new PageImpl<>(result.subList(start, end), pageable,
        result.size());
  }
  ...
  public ProductDTO findById(Long id) {
    ...
    return productMapper.getProductDTOFromEntity(resultQuery);
  }
  ...
  public ProductEntity saveNewProduct(ProductDTO productDTO) {
    ...
    ProductTempDTO productTempDTO=mapper.getProductTempDTOFromProductEntity(productEntity);
    productTempDTO.setProductId(id);
    productTempDTO.setState(ProductStateEnum.CREATE.getValue());
    productTempService.saveNewProduct(productTempDTO);
    return entity;
  }
  ...
  public ProductEntity updateProduct(ProductDTO productDTO, long id) {
    ...
    ProductTempDTO productTempDTO=mapper.getProductTempDTOFromProductEntity(productEntity);
    productTempDTO.setProductId(id);
    productTempDTO.setState(ProductStateEnum.UPDATE.getValue());
    productTempService.saveNewProduct(productTempDTO);
    return entity;
  }
  ...
  public void delListProductByListId(Long[] ids) {
    ...
    Arrays.stream(ids).forEach(id ->{
      ...
      ProductTempDTO productTempDTO=mapper.getProductTempDTOFromProductEntity(entity.get());
      productTempDTO.setProductId(id);
      productTempDTO.setState(ProductStateEnum.DELETE.getValue());
      productTempService.saveNewProduct(productTempDTO);
    });
  }
